...
const EditInvoice = () => {
    ...
    useEffect(() => {
        const fetchCountries = async () => {
            try {
                ...
                }).filter(country => country.currencyCode); // Filter out countries without currency
    ...
    useEffect(() => {
        fetch(`http://localhost:8000/api/invoices/${invoiceId}/`)
            ...
            .then((data) => {
                ...
                // Set selected country based on saved currency
                if (data.currency) {
                    const foundCountry = countries.find(c => c.currencyCode === data.currency) ||
                        { name: "India", currency: "₹", currencyCode: "INR" };
    ...
    useEffect(() => {
        fetch("http://127.0.0.1:8000/api/settings/")
            .then((res) => res.json())
            .then((data) => {
                setSettingsData(Array.isArray(data) ? data[0] : data);
            })
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const res = await fetch(`http://localhost:8000/api/update/${invoiceId}/`, {
                method: "PUT",
                headers: {
    ...
    useEffect(() => {
        calculateTotal();
    }, [formData.total_hours, formData.rate, selectedCountry.name]);
    ...
    const calculateTotal = () => {
        ...
        if (selectedCountry.name === "India" && base_amount > 0) {
            ...
            const total_tax = totalTax(tax, tax);
    ...
    useEffect(() => {
        fetch("http://127.0.0.1:8000/api/settings/")
            ...
            .then(async (data) => {
                ...
                if (setting?.logo) {
                    const base64Logo = await toBase64(`http://127.0.0.1:8000${setting.logo}`);
    ...
    return (
        <div style={{ padding: "10px 15px 10px 85px" }}>
            <h2 className="text-center">TAX INVOICE</h2>
            <form onSubmit={handleSubmit}>
                <div style={{ border: "2px solid", backgroundColor: "white", padding: "15px" }}>
                        ...
                        <div className="col-6">
                            <table className="table black-bordered">
                                <tbody>
                                    <tr>
                                        <td className="gray-background">
                                            <strong style={{ fontSize: "15px" }}>Grabsolve Infotech:</strong>
                                        </td>
                                    </tr>
                                    {settingsData && (
                                        <>
                            ...
                            <table className="table black-bordered">
                                ...
                                <tbody>
                                    <tr style={{ height: "130px" }}>
                                        ...
                                        <td>{formData.rate}</td>
                                        <td><span className="currency-sym">{selectedCountry.currency}</span>{formData.base_amount}</td>
                                    </tr>
                                    {selectedCountry.name === "India" && (
                                        <>
                                            <tr><td></td><td><span style={{ float: "right" }}>CGST @ 9%</span></td><td></td><td></td><td>9%</td><td><span className="currency-sym">₹</span>{formData.cgst}</td></tr>
                                            <tr><td></td><td><span style={{ float: "right" }}>SGST @ 9%</span></td><td></td><td></td><td>9%</td><td><span className="currency-sym">₹</span>{formData.sgst}</td></tr>
                                        </>
                                    )}
                                    <tr>
                                        <td colSpan="5" className="text-right"><strong>Total</strong></td>
                                        <td><strong><span className="currency-sym">{selectedCountry.currency}</span> {formData.total_with_gst}</strong></td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
                    ...
                    {selectedCountry.name === "India" && (
                        <div className="row" style={{ padding: "10px" }}>
                            <div className="col-12">
                                ...
                                <strong>Tax Amount (in words):</strong>
                                <span className="total-tax-in-words">INR {numberToWords(Math.floor(formData.total_with_gst))}</span>
                            </div>
